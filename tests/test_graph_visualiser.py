...
def test_the_bridge_translates_in_both_directions(executor, sample_iga):
    ...
    by_iga = build_ego(sample_iga, executor=executor)
...
def test_a_table_is_probed_on_its_source_key_not_any_id_column(executor, sample_iga):
    """`EMPLOYEE_INFO` carries `EMPLOYEE_ID` as well as `IGA`, and counting on
    the wrong one answers a different question."""
    from crew_perf.graph_visualiser.crew_ego import build_ego

    ego = build_ego(sample_iga, executor=executor)
...
def test_a_source_holding_nothing_is_named_as_missing_data(executor):
    ...
    # A crew member the fleet holds but a source does not. The extracts are now
    # cut to the fleet, so nobody is missing from ServiceNow or CAC — the gap
    # this view exists to name is the one the generator injects deliberately,
    # where a crew member was never onboarded into CLMS.
    iga = executor.execute("""
        SELECT e.IGA FROM EMPLOYEE_INFO e
        WHERE e.P_IS_CURRENT = TRUE
          AND NOT EXISTS (
              SELECT 1 FROM M_CREW_DETAILS d
              JOIN M_CLMS_CREW c ON c.CREW_ID = d.CREW_ID
              WHERE d.IGA = e.IGA AND d.P_IS_CURRENT = TRUE)
        ORDER BY e.IGA LIMIT 1
    """, limit=1).rows[0][0]

    ego = build_ego(iga, executor=executor)
    assert ego["found"]
    assert ego["absent_sources"], f"{iga} should be absent from at least one source"
...
def test_the_crew_lookup_survives_a_concurrent_request(client, sample_iga):
    ...
    expected = client.get(f"/api/graph-view/crew/{sample_iga}").json()
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        futures = [
            pool.submit(hit, f"/api/graph-view/crew/{sample_iga}" if i % 2 else "/api/overview")
            for i in range(12)
        ]
