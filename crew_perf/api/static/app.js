...
/* The narrative comes back as prose with the occasional bold run, bullet
 * list, or — when the model is comparing rows it pulled together itself
 * rather than handing back one query's results — a markdown table. A full
 * markdown library is a dependency for four constructs, and the input is
 * model output rendered into a page — so it is escaped FIRST and the four
 * constructs are re-introduced afterwards. Nothing else can become markup,
...
function renderProse(text) {
   ...
   * recognised by the separator line alone since dashes-and-pipes is not a
   * shape prose ever takes on its own. */
  ...
  /* A column where every value parses as a number is shown right-aligned in
   * tabular figures, like the rest of the page's numeric columns — and each
   * value is rounded rather than reproduced at whatever precision the model
   * happened to compute at (`98.50166666666667` reads as noise, not data). */
  function renderTable({ header, aligns, rows }) {
    const numeric = header.map((_, i) =>
      rows.length > 0 && rows.every((r) => isNumeric(r[i])));
    const cell = (raw, i, tag) => {
      const num = numeric[i];
      const align = num ? "right" : aligns[i];
      const attrs = num ? ' class="num"' : align ? ` style="text-align:${align}"` : "";
      const body = raw === undefined ? "" : num ? formatNumber(raw) : inline(raw);
      return `<${tag}${attrs}>${body}</${tag}>`;
    };
    const thead = `<tr>${header.map((h, i) => cell(h, i, "th")).join("")}</tr>`;
    const tbody = rows.map((r) =>
      `<tr>${header.map((_, i) => cell(r[i], i, "td")).join("")}</tr>`).join("");
    return `<div class="tablewrap"><table><thead>${thead}</thead>`
      + `<tbody>${tbody}</tbody></table></div>`;
  }

  function isNumeric(s) {
    return typeof s === "string" && s.trim() !== "" && /^-?[\d,]+(\.\d+)?%?$/.test(s.trim());
  }
  ...
  function formatNumber(raw) {
    ...
    if (Number.isNaN(n)) return escapeHtml(s);
    const shown = Number.isInteger(n) ? String(n) : n.toFixed(2).replace(/\.?0+$/, "");
    return escapeHtml(shown + pct);
  }
}

/* The answer itself lands as one complete string the instant the pipeline
 * finishes — stage events already stream live, but that final string does
 * not, so dumping it in one frame reads as a jolt after the live progress
 * that came before it. This reveals it as if it were still being written,
 * over a duration capped so a long answer never drags the wait. Everything
 * that follows the prose (crew cards, table, the detail toggles) waits for
 * `done` so it never appears mid-sentence. */
function useReveal(pending, text) {
  const [shown, setShown] = useState(0);
  const [done, setDone] = useState(false);
  const startedRef = useRef(false);

  useEffect(() => {
    if (pending || !text || startedRef.current) return;
    startedRef.current = true;
    const total = text.length;
    const duration = Math.min(1400, Math.max(350, total * 3.2));
    const start = performance.now();
    let frame = requestAnimationFrame(function tick(now) {
      const t = Math.min(1, (now - start) / duration);
      setShown(Math.round(t * total));
  ...
  return {
    shown: text ? text.slice(0, shown) : "",
    done: pending ? false : (done || !text),
  };
}

/* A reveal can land mid-way through a `**bold**` marker. Rendered as-is that
 * is a naked pair of asterisks for one frame; trimmed back to the last
 * complete pair instead, so the tag only appears once its text has. */
function trimDangling(s) {
  const pairs = (s.match(/\*\*/g) || []).length;
  return pairs % 2 === 1 ? s.slice(0, s.lastIndexOf("**")) : s;
}
...
function Detail({ detail, crew, reveal }) {
  ...
  return html`
    <div class="detail ${reveal ? "reveal-in" : ""}">
...
function Message({ msg }) {
  ...
  // stays fixed across renders — a user bubble never touches its result.
  const { shown, done } = useReveal(msg.role === "bot" && msg.pending, msg.answer);
  ...
  const prose = renderProse(msg.pending ? "" : (done ? msg.answer : trimDangling(shown)));
  const revealRest = !msg.pending && done;
  return html`
    <div class="msg">
        ...
        ${revealRest ? html`
          <${Fragment}>
            ${msg.crew?.length ? html`
              <div class="cards reveal-in">
                ${msg.crew.map((m) => html`<${CrewCard} key=${m.iga} member=${m} />`)}
              </div>` : null}
            <${ResultTable} table=${msg.table} />
            <${Detail} detail=${msg.detail} crew=${msg.crew} reveal=${true} />
          <//>` : null}
      </div>
    </div>`;
}
