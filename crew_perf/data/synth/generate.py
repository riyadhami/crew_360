...
FIRST_NAMES = [
    "Aarav", "Aditi", "Akash", "Ananya", "Arjun", "Bhavna", "Chirag", "Deepika",
    "Dev", "Divya", "Farhan", "Gaurav", "Harini", "Ishaan", "Jaya", "Kabir",
    "Kavya", "Manish", "Meera", "Naveen", "Neha", "Nikhil", "Pooja", "Pranav",
    "Priya", "Rahul", "Riya", "Rohit", "Sanjay", "Shreya", "Siddharth", "Sneha",
    "Tanvi", "Varun", "Vikram", "Vivek", "Yash", "Zoya", "Ritika", "Imran",
]
LAST_NAMES = [
    "Agarwal", "Bansal", "Chopra", "Desai", "Gupta", "Iyer", "Jain", "Kapoor",
    "Khanna", "Kulkarni", "Malhotra", "Mehta", "Menon", "Nair", "Patel", "Rao",
    "Reddy", "Saxena", "Sharma", "Singh", "Sinha", "Verma", "Joshi", "Pillai",
]
...
class PepGenerator:
    ...
    def build_crew(self) -> None:
        ...
        igas = [f"IGA{60000 + i}" for i in range(n)]
        designation = np.where(rng.random(n) < P_LEAD, "LD", "CA")
        fleet = np.where(rng.random(n) < P_ATR, 2, 1)  # flight type id
        ...
        self._df("EMPLOYEE_INFO", [
            {"EMPLOYEE_ID": 100000 + i, "IGA": igas[i], "EMPLOYEE_NAME": names[i],
             "BASE": base[i], "DESIGNATION": designation[i],
             "EMAIL_ID": names[i].lower().replace(" ", ".") + "@airline.com",
        ...
        self.latent = pd.DataFrame({
            "IGA": igas, "EMPLOYEE_NAME": names, "DESIGNATION": designation,
            "BASE": base, "FLIGHT_TYPE": fleet, "ACTIVE": active,
            "general_factor": g,
        ...
        mentor_names, mseen = [], set()
        while len(mentor_names) < n_mentors:
            nm = f"{rng.choice(FIRST_NAMES)} {rng.choice(LAST_NAMES)}"
            if nm not in mseen:
                mseen.add(nm)
                mentor_names.append(nm)
        self.mentors = pd.DataFrame({
            "name": mentor_names,
            "login": [nm.lower().replace(" ", ".") for nm in mentor_names],
            "email": [nm.lower().replace(" ", ".") + "@airline.com" for nm in mentor_names],
    ...
    def build_assessments(self) -> None:
        ...
        for idx, crew in enumerate(latent.itertuples()):
            ...
            for _ in range(n_assessments[idx]):
                ...
                sched_rows.append({
                    ...
                    "PEP_REASON": reason, "MENTOR_NAME": mentor["name"],
                ...
                planned_rows.append({
                    ...
                    "DESIGNATION": crew.DESIGNATION, "CREW_NAME": crew.EMPLOYEE_NAME,
                    "FLIGHT_NO": flight_no, "PEP_DATE": pep_date, "PEP_REASON": reason,
                    "MENTOR_LOGIN": mentor["login"], "MENTOR_NAME": mentor["name"],
                })
                planned_id += 1
        ...
        for _ in range(n_changes):
            ...
            change_rows.append({
                ...
                "CHANGE_DATE": s["PEP_DATE"], "MENTOR_NAME": s["MENTOR_NAME"],
            })
            change_id += 1
        for _ in range(n_changes // 2):
            ...
            interchange_rows.append({
                ...
                "MENTOR_CHANGE_NAME": m["name"], "MODIFIED_ON": self.cfg.end_date,
            })
            inter_id += 1
