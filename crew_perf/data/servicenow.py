...
def load(path: Path | None = None, executor=None) -> IngestResult:
    ...
    from crew_perf.data import extracts
    ...
    try:
        # Cut to the generated fleet. Without this the database describes two
        # populations — the crew this system can score, and everyone else the
        # extract happens to mention — and every count becomes ambiguous.
        cut = extracts.restrict(frames, extracts.fleet_igas(executor))
        if cut["dropped"]:
            # A report keeps its own grain: it is dropped only when the cut left
            # it naming nobody in the fleet at all.
            orphaned = extracts.cascade(
                frames, ("SN_FLIGHT_REPORT", "SN_SERVICE_CHECK"),
                "SN_REPORT_CREW", "SN_REPORT_ID")
            result.findings.append(
                f"{cut['dropped']:,} crew row(s) name someone outside the fleet and "
                f"{orphaned:,} report(s) then named nobody in it — both are dropped. "
                f"The extract covers {cut['crew_before']:,} crew; this fleet is "
                f"{cut['crew_after']:,} of them."
            )
    ...
    # Recount after the fleet cut, or the summary reports what was parsed rather
    # than what was loaded — the two differ by two thirds here.
    result.tables = {name: len(rows) for name, rows in frames.items()}
